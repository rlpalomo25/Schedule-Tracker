  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-2xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
          <div>
            ...
            <p className="text-[11px] text-slate-500">Live headcount capacity per hour across all global tiers</p>
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden">
        ...
        <div className="sticky top-14 z-20 bg-slate-900 text-slate-200 px-4 py-2.5 flex items-center border-b border-slate-800 text-xs font-semibold">
          {/* Employee Info Header Column */}
          <div className="w-64 sm:w-72 shrink-0 pr-4">
            Team Member & Details
          </div>
          ...
          <div className="flex-1 relative flex">
            {hoursArray.map((hour) => (
              <div
                key={hour}
                className="flex-1 text-center font-mono text-[11px] text-slate-400 border-l border-slate-800/80"
              >
          ...
          {filteredEmployees.length === 0 ? (
            ...
            filteredEmployees.map((emp) => {
              ...
              return (
                <div
                  key={emp.id}
                  className={`flex items-center px-4 py-2.5 hover:bg-slate-50/80 transition-colors group relative ${
                    isMyProfile ? 'bg-indigo-50/40 font-medium' : ''
                  }`}
                >
                  {/* Employee Info Box */}
